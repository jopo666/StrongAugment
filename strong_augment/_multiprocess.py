...
def multiprocess_loop(
    ...
    **kwargs,
    ...
    Args:
        ...
        num_workers: Number of worker processes. If None, set to the number of CPU
            cores on the machine. Defaults to None.
        initializer: Initializer function for each worker process. Defaults to None.
        initializer_args:: Arguments for the `initializer` function. Defaults to ().
        use_imap: Uses imap instead of map. Imap returns results in the same order but
            is slightly slower to start up. Defaults to True.
